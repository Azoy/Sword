...
public struct AuditLog {
  ...
  public internal(set) var entries = [AuditLogEntry]()
  ...
  */
  init(_ sword: Sword, _ json: [String: [Any]]) {
    ...
    for entry in entries {
      self.entries.append(AuditLogEntry(entry as! [String: Any]))
    }
...
public struct AuditLogEntry {
  ...
  public let actionType: AuditLogEvent
  
  /// Array of changes made to targetId
  public internal(set) var changes = [AuditLogChange]()
  ...
  */
  init(_ json: [String: Any]) {
    self.actionType = AuditLogEvent(rawValue: json["action_type"] as! Int)!
    
    let changes = json["changes"] as! [[String: Any]]
    for change in changes {
      self.changes.append(AuditLogChange(change))
    }
