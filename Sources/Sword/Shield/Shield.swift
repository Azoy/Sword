...
open class Shield: Sword {
  ...
  func handle(message data: [Any]) {
    ...
    for prefix in self.shieldOptions.prefixes {
      ...
      if let normalCommand = self.commands[command.lowercased()] {
        if self.shieldOptions.isCaseSensitive == normalCommand.options.overwriteCaseSensitive {
          command = command.lowercased()
        }
      } else if let normalCommandAlias = self.commandAliases[command.lowercased()] {
        if let normalCommand = self.commands[normalCommandAlias] {
          if self.shieldOptions.isCaseSensitive == normalCommand.options.overwriteCaseSensitive {
            command = command.lowercased()
          }
        }
  ...
  public func register(_ commandName: String, with options: CommandOptions = CommandOptions(), _ function: @escaping (Message, [String]) -> ()) {
    if self.shieldOptions.isCaseSensitive == options.overwriteCaseSensitive {
      let lowerCasedCommandName = commandName.lowercased()
      self.commands[lowerCasedCommandName] = Command(name: lowerCasedCommandName, function: function, options: options)
    
      if !options.aliases.isEmpty {
        for alias in options.aliases {
          self.commandAliases[alias.lowercased()] = lowerCasedCommandName
        }
      }
    } else {
  ...
  public func unregister(_ commandName: String) {
    ...
    if let command = self.commands[commandName] {
      options = command.options
    } else if let command = self.commands[commandName.lowercased()] {
      options = command.options
    }
    if self.shieldOptions.isCaseSensitive == options.overwriteCaseSensitive {
      let lowerCasedCommandName = commandName.lowercased()
      for (key, value) in self.commandAliases {
        if value == lowerCasedCommandName {
          self.commandAliases.removeValue(forKey: key)
        }
      }
      self.commands.removeValue(forKey: lowerCasedCommandName)
    } else {
      for (key, value) in self.commandAliases {
